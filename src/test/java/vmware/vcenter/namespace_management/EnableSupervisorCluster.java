 ...
 */
package vmware.vcenter.namespace_management;
...
import vmware.common.SamplesAbstractBase;
import vmware.vcenter.helpers.ClusterHelper;

/**
 * Description: Demonstrates how to enable vSphere supervisor cluster on given cluster (NSX-T should be configured)
 *
 * Author: Vikas Shitole, VMware, vThinkBeyondVM.com Sample Prerequisites: All below params need to be passed for this sample
 * Understand APIs: https://vthinkbeyondvm.com/script-to-configure-vsphere-supervisor-cluster-using-rest-apis/
 * Offcial doc: https://docs.vmware.com/en/VMware-vSphere/7.0/vmware-vsphere-with-kubernetes/GUID-8D0E905F-9ABB-4CFB-A206-C027F847FAAC.html
 */
public class EnableSupervisorCluster extends SamplesAbstractBase {
	...
	protected void parseArgs(String[] args) {
		...
		Option clusterNameOption = Option.builder().longOpt("clustername")
				.desc("REQUIRED: Name of the cluster we need to get info" + " about k8scluster").required(true).hasArg()
				.argName("Cluster Name").build();
		Option hintSizeOption = Option.builder().longOpt("hintsize")
				.desc("REQUIRED: Enum value as either of TINY/SMALL/MEDIUM/LARGE").argName("Cluster size")
				.required(true).hasArg().build();
		Option masterVmNetworkOption = Option.builder().longOpt("masternetwork")
				.desc("REQUIRED: Master VM network port group").argName("Master Network").required(true).hasArg()
				.build();
		Option floatingipOption = Option.builder().longOpt("floatingip")
				.desc("REQUIRED: Floating IP used by HA master cluster ").argName("Master starting IP").required(true)
				.hasArg().build();
		Option masterdnsserverOption = Option.builder().longOpt("masterdnsserver").desc("Master DNS server IP ")
				.argName("DNS Server").required(true).hasArg().build();
		Option workerdnsserverOption = Option.builder().longOpt("workerdnsserver").desc(" Worker DNS server IP ")
				.argName("DNS Server").required(true).hasArg().build();
		Option ntpserverOption = Option.builder().longOpt("ntpserver").desc("NTP server IP ").argName("NTP Server")
				.required(true).hasArg().build();
		Option storagepolicyOption = Option.builder().longOpt("storagepolicy")
				.desc("REQUIRED: Storage Policy for Master, image and ephermal storage ").argName("Storage Policy")
				.required(true).hasArg().build();
		Option podcidrOption = Option.builder().longOpt("podcidr")
				.desc("REQUIRED: POD cidr network i.e. 10.2.3.4/24 (including prefix)").argName("POD CIDR")
				.required(true).hasArg().build();
		Option servicecidrOption = Option.builder().longOpt("servicecidr")
				.desc("REQUIRED: Service cidr  i.e. 10.2.3.4/21  (including prefix)").argName("Service CIDR")
				.required(true).hasArg().build();
		Option ingresscidrOption = Option.builder().longOpt("ingresscidr")
				.desc("REQUIRED: Ingress cidr network (including prefix) ").argName("Ingress CIDR").required(true)
				.hasArg().build();
		Option egresscidrOption = Option.builder().longOpt("egresscidr")
				.desc("REQUIRED: Egress cidr network (including prefix)").argName("Egress CIDR").required(true).hasArg()
				.build();

		List<Option> optionList = Arrays.asList(clusterNameOption, hintSizeOption, masterVmNetworkOption,
				floatingipOption, masterdnsserverOption, workerdnsserverOption, ntpserverOption, storagepolicyOption,
				podcidrOption, servicecidrOption, ingresscidrOption, egresscidrOption);
	...
	protected void setup() throws Exception {
		...
		// Getting VDS uuid
		this.VDSService = this.vapiAuthHelper.getStubFactory().createStub(DistributedSwitchCompatibility.class,
		...
		this.nsxEdgeService = this.vapiAuthHelper.getStubFactory().createStub(EdgeClusterCompatibility.class,
				this.sessionStubConfig);
		...
		List<EdgeClusterCompatibilityTypes.Summary> edgeSummary = this.nsxEdgeService.list(this.clusterId,
				vdsSummary.get(0).getDistributedSwitch(), edgeFilter);
		...
		this.networkService = this.vapiAuthHelper.getStubFactory().createStub(Network.class, this.sessionStubConfig);
	...
	protected void run() throws Exception {
		...
		// un-comment below code if you want to enter static IP range. Production
		// enviornment should use it
		/*
